...
| Gathering hints | ✅ | Words one honest entry short of a finding — exactly two distinct entries across two distinct days, recent, confidence-held, and not already a held pattern — surface as a quiet "Still gathering" section on both Patterns screens stating the gap ("2 entries on 2 days · a finding needs 3 across 2"). Computed on request (`GET /v1/patterns/gathering`), stores nothing, cannot become a finding by being looked at; echoes, stale evidence, weak readings, and lapsed patterns' current holdings stay silent. The floor only spoke when crossed, and the silence below it read as "nothing is here" — itself a false claim |
| Threads | ✅ | Findings that rest on the same word, grouped for navigation — arithmetic over stored SUPPORTS labels, no new claim. A thread names its shared subjects in the record's own phrasing, may chain through a finding that bridges two subjects (both named), and keeps every member's own claim, confidence, and way in. `GET /v1/patterns/threads`; shown on the web and mobile Patterns screens, gated behind the findings switch like every other conclusions view. Recent temporal shifts whose label matches a subject join the card as a "changed lately" row (exact normalised-label match only, linking to Week) — the freshest signal without cross-referencing two screens |
...
| e2e suite | ✅ | `scripts/e2e.sh` drives the mobile build in a real browser; 137 checks, all passing, credentials cleared so runs stay deterministic. Not immune to a cold web bundle: a first run after a rebuild has been seen to fail the Skia canvas and the first explain-screen click, then pass unchanged on a re-run. Treat a single red run as unconfirmed |
| web e2e suite | ✅ | `scripts/e2e-web.sh` drives the desktop client the same way — signup through the form, save → extract → readings drawn behind the save, provenance screen, patterns' honest empty state, an experiment written and started, search over the person's own words, findings-off contract (guarded routes redirect home), and a typed Talk turn answered with the loop kept alive. 41 checks; the voice 503 is named as allowed, since stripping the credential makes it the correct answer |
...
| Client | Stack | State |
|---|---|---|
| `apps/web` | Vite + React + TypeScript | The Open Design UX (`tlon.html`) ported and wired to the live API, with its own browser suite (`scripts/e2e-web.sh`, 23 checks): signup, journaling with extraction, provenance, patterns, threads, Talk (typed turns answered; TTS endpoint wired), findings-off contract, graph. The full 3D Headspace treatment remains the one visual gap |
