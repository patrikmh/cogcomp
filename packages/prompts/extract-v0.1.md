...
Turns one raw observation into candidate graph nodes and edges.

This file is the prompt. It is not assembled from fragments at runtime and there is
no hidden preamble — what is below is what the model receives, with the observation
appended as the user turn. The `extractor` field on every node this produces records
`extract-v0.1/<model-id>`, so any output can be traced back to this exact text.
...
|---|---|---|
| model | `claude-opus-5` | |
| effort | `medium` | Extraction is bounded and well-specified; higher effort mostly buys deliberation this task doesn't need. |
| thinking | default (on) | Opus 5 thinks by default. Disabling it is the documented cause of malformed output on this model. |
| output_config.format | `json_schema` | The schema is the contract; a response that doesn't validate is discarded rather than repaired. |
| max_tokens | 16000 | Non-streaming, so kept under the SDK timeout threshold. |

Sampling parameters are absent because Opus 5 rejects them.
...
Defined in `apps/backend/src/extraction/mod.rs` as the single source of truth and sent
with each request. Confidence bounds are not expressed in the schema — the JSON Schema
subset the API accepts does not support numeric ranges — so they are enforced by the
`Confidence` type on the way in.

## Changelog

- **v0.1** — first version. Milestone 1.
