...
import { ApiError, api, type Experiment, type GraphNode, type Pattern, type ObservationResponse } from "@/lib/api";
import { eligibleHeld } from "@/lib/drawnFrom";
...
export default function ExperimentDetail() {
  ...
  const graph = useQuery({ queryKey: ["graph", userId], queryFn: () => api.graph(token!, { limit: 200 }), enabled: Boolean(token && userId && query.data?.state === "draft") && findingsVisible });
  ...
  return <AtmosphericShell variant="secondary"><ScrollView contentContainerStyle={styles.screen} accessibilityLabel="Experiment detail">
    ...
    {findingsVisible && experiment.state === "draft" && (() => {
      const taken = new Set((experiment.links ?? []).map((linked: { node_id: string }) => linked.node_id));
      const held = ((graph.data?.nodes ?? []) as GraphNode[]).filter((reading) => eligibleHeld(reading) && !taken.has(reading.id));
      const live = (patterns.data ?? []).filter((pattern: Pattern) => eligiblePattern(pattern) && !taken.has(pattern.id));
      return <View style={styles.section}><Text style={styles.heading}>Set it against</Text><Text style={styles.meta}>A reading you hold, or a live pattern. Yours, not proposed.</Text>{(patterns.isLoading || graph.isLoading) && <ActivityIndicator accessibilityLabel="Loading readings to set this against" color={colors.cyan} />}{held.map((reading) => <Pressable key={reading.id} accessibilityRole="button" accessibilityLabel={`Link reading ${reading.label}`} disabled={link.isPending} onPress={() => link.mutate(reading.id)} style={styles.pattern}><Text style={styles.body}>{reading.label}</Text><Text style={styles.meta}>{reading.kind.toLowerCase()} · you hold this</Text></Pressable>)}{live.map((pattern: Pattern) => <Pressable key={pattern.id} accessibilityRole="button" accessibilityLabel={`Link pattern ${pattern.label}`} disabled={link.isPending} onPress={() => link.mutate(pattern.id)} style={styles.pattern}><Text style={styles.body}>{pattern.label}</Text><Text style={styles.meta}>Live pattern · {pattern.occurrences} occurrences</Text></Pressable>)}{!patterns.isLoading && !graph.isLoading && held.length === 0 && live.length === 0 && <Text style={styles.meta}>Nothing live to set this against yet.</Text>}</View>;
    })()}
