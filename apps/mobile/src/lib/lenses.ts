 ...
 *
 * Two of these show what you wrote; two show what the system concluded about
 ...
 * fact, the whole graph is a record, a recurrence is a claim, and a change is a
 * comparison between two windows. Each step is further from what was actually
 * written, and the row makes that distance visible rather than presenting all
 * four as equally solid.
 *
 * Regions used to be a fifth lens. They are a finding like any other and now
 * sit on Patterns beside the recurrences, which is where the design and the web
 * client both put them — a region is a group that keeps turning up together,
 * the same kind of claim as a recurrence, and it belongs on the same page. A
 * lens of its own also made the row the one place regions could be reached,
 * which is why it stayed longer than it should have.
 */
export type Lens = "today" | "all" | "patterns" | "changed";
